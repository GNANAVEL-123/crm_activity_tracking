...
def send_sales_invoice_whatsapp(invoice):
	...
	frappe.enqueue(
				...
				doc=doc,
				message=f"Sales Invoice - {doc.posting_date}",
				invoice=doc.name
			)
	return 'Success'
...
def send_whatsapp(media_url, message, doc, invoice=None):
    ...
    # customer_mobile_no=frappe.db.get_value("Customer", doc.customer, "mobile_no")
    ...
    if customer_mobile_no:
            ...
            url_message = f"{url}FileWithCaption?token={instance_id}&phone=91{customer_mobile_no}&link={media_url}&message={message}"
...
def send_task_message(invoice):
    ...
    if task_doc and task_doc.custom_allocated_to:
        ...
        from urllib.parse import quote  # Add at the top of the file
