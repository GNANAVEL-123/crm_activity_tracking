...
def execute(filters=None):
	...
	for cust in customers:
		if cint(cust[8]) >= cint(days_since_last_order):
			...
			# core edit start
			email, phone = get_customer_contact_details(cust[0])
			cust.append(email)
			cust.append(phone)
			# core edit end
			data.append(cust)
...
def get_customer_contact_details(customer):
    ...
    email = frappe.db.get_value("Address", address, "email_id") or ""
    phone = frappe.db.get_value("Address", address, "phone") or ""
